// Compile (MSYS2)      : $ g++ -std=c++14 -I"C:\Users\RJW\vcpkg\packages\libpcap_x64-windows\include" -L"C:\Users\RJW\vcpkg\packages\libpcap_x64-windows\lib" -o Backtest ./Backtest.cpp -lpcap -O2
// Run     (Powershell) : ./Backtest ..\data/md-cfe-tsp05-eth7-d-2024-41-ok.pcap
...
// Global variables
bool gGapDetected{ false };                        // Gap detected flag
uint32_t gNextExpectedPacketSeqNum{ 1 };           // Next expected packet sequence number
string currentDate;                                // Current date
string currentTradeDate;                           // Current trade date
uint32_t currentTimeInSecs;                        // Number of whole seconds from midnight CentralTime
unordered_map<uint8_t, int> messageCounts;         // Message counts by type
unordered_map<string, unordered_map<uint8_t, int>> dailyMessageCounts; // Messages per day and type
int totalMessages = 0;                             // Total messages processed
chrono::duration<double> duration_process(0);      // Cumulative duration of processing messages
OrderManager orderManager;				   		   // Order manager
...
void getTimestamp(char* buffer, const std::string& Date, uint32_t timeInSecs, uint32_t timeOffset) {
    // Extract year, month, and day directly from the Date string (assuming "YYYYMMDD" format)
    ...
    std::snprintf(buffer, 30, "%04d-%02d-%02d %02d:%02d:%02d.%09u",
...
int process_message(const uint64_t pktSeqNum, uint64_t msgSeqNum, const u_char* message, int msg_type)
{
    ...
    totalMessages++;
    messageCounts[msg_type]++;

    // Ensure the date entry exists in the map
    if (dailyMessageCounts.find(currentTradeDate) == dailyMessageCounts.end()) {
        dailyMessageCounts[currentTradeDate] = unordered_map<uint8_t, int>();
    }
    dailyMessageCounts[currentTradeDate][msg_type]++;
    ...
    case 0xB1: // TimeReference
    {
        ...
        */
		...
		//Extract the current trade date(YYYYMMDD)
        currentTradeDate = to_string(m.TradeDate);
        
		// Parse the midnight reference date(YYYYMMDD)
        std::time_t midnightRefTime = m.MidnightReference;
        ...
        orderManager.updateCurrentDate(currentDate);
        orderManager.updateCurrentTradeDate(currentTradeDate);
    ...
    case 0xBB: // FuturesInstrumentDefinition
    {
        ...
        */
		...
		orderManager.symbolMap.updateSymbolMap(m, message);
    ...
    case 0x31: // TradingStatus
    {
        ...
        */
		...
		orderManager.updateTradingStatus(string(reinterpret_cast<const char*>(m.Symbol), 6), m.TradingStatus);
    ...
    case 0x21: // AddOrderLong
    {
        ...
        Order order = { m.OrderId, m.SideIndicator, m.Quantity, m.Symbol, m.Price };
        
        char srcTime[29];
        getTimestamp(srcTime, currentDate, currentTimeInSecs, m.TimeOffset);

        orderManager.addOrder(srcTime, pktSeqNum, msgSeqNum, order);
    ...
    case 0x22: // AddOrderShort
    {
        ...
        */
        ...
        Order order = { m.OrderId, m.SideIndicator, static_cast<uint32_t>(m.Quantity), m.Symbol, static_cast<int64_t>(m.Price) };
    ...
    case 0x23: // OrderExecuted
    {
        ...
        */
        ...
        orderManager.executeOrder(srcTime, pktSeqNum, msgSeqNum, m.OrderId, m.ExecutedQuantity);
    ...
    case 0x25: // ReduceSizeLong
    {
        ...
        orderManager.reduceSize(srcTime, pktSeqNum, msgSeqNum, m.OrderId, m.CancelledQuantity);
    ...
    case 0x26: // ReduceSizeShort
    {
        ...
        */
        ...
        orderManager.reduceSize(srcTime, pktSeqNum, msgSeqNum, m.OrderId, static_cast<uint32_t>(m.CancelledQuantity));
    ...
    case 0x27: // ModifyOrderLong
    {
		...
		orderManager.modifyOrder(srcTime, pktSeqNum, msgSeqNum, m.OrderId, m.Quantity, m.Price);
    ...
    case 0x28: // ModifyOrderShort
    {
        ...
        */
		...
		orderManager.modifyOrder(srcTime, pktSeqNum, msgSeqNum, m.OrderId, static_cast<uint32_t>(m.Quantity), static_cast<int64_t>(m.Price));
    ...
    case 0x29: // DeleteOrder
    {
        ...
        */
		...
		orderManager.deleteOrder(srcTime, pktSeqNum, msgSeqNum, m.OrderId);
...
int process_packet(const u_char* packet, struct pcap_pkthdr header)
{
    ...
    */
    ...
    if (gNextExpectedPacketSeqNum)
    {
        ...
        {
            cout << "Packet Processor: feed startup detected" << endl;
        }
...
// Write the message summary to a CSV file
void writeMessageSummaryToCsv(const string& filename) {
    ...
    csvFile << "Date,Message Type,Message Type Hex,Count\n";
    for (const auto& dateEntry : dailyMessageCounts) {
        for (const auto& typeEntry : messageTypeInfo) {
            auto it = dateEntry.second.find(typeEntry.first);
            int count = (it != dateEntry.second.end()) ? it->second : 0;
            csvFile << dateEntry.first << ","
                << typeEntry.second << ","
                << "0x" << hex << setw(2) << setfill('0') << static_cast<int>(typeEntry.first) << dec << ","
                << count << "\n";
        }
    }

    csvFile.close();
    cout << "Messages Summary written to " << filename << endl;
}
...
int main(int argc, char* argv[]) {
	...
	orderManager.symbolMap.loadSymbolMapFromCsv("symbol_map.csv");
    std::vector<std::pair<std::string, std::string>> VX_VXM_symbolPairs = orderManager.symbolMap.getVX_VXM_symbolPairs();

    // For testing
    // A basic sanity check for arbitrage t
    // rades across different products is to ensure that 
    // the aggregated individual trade logs from a pair arbitrage is the same as the total trade log.
	 //string symbol1 = orderManager.symbolMap.getSymbolInfoByName("VXMV4").symbolId;
     //string symbol2 = orderManager.symbolMap.getSymbolInfoByName("VXV4").symbolId;
	 //VX_VXM_symbolPairs = { {symbol1, symbol2} };

    Trader trader(orderManager, VX_VXM_symbolPairs);
    orderManager.registerObserver(&trader);
    ...
    // Output the summary
    if (!gGapDetected) {
        cout << "No gap detected" << endl;
    }

    cout << "Total messages processed: " << totalMessages << endl
        << "Total Time taken to process messages: " << duration_cast<seconds>(duration_process).count() << " s " << endl << endl;
    writeMessageSummaryToCsv("message_summary.csv");
    orderManager.writeOrderStoreToCsv("order_store.csv");
    orderManager.writeOrderBookToCsv("order_book.csv");
	orderManager.writeBBOToCsv("bbo_history.csv");
	orderManager.symbolMap.writeSymbolMapToCsv("symbol_map.csv");
    orderManager.writeTradeVolumeToCsv("trade_volume.csv");
	trader.writeTradeLogToCsv("trade_log.csv");
